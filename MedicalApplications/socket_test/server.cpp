...
int main() {
    ...
    reader = read(obj_socket, buffer, 65535);
    data_stream << buffer;
    ...
    cout << "Set up encoder and evaluator" << endl;
    // Set up encoder and evaluator
    CKKSEncoder encoder(context);
    Evaluator evaluator(context);

    // Read ciphertext
    cout << "Read ciphertext from bergman" << endl;
    reader = read(obj_socket, buffer, 65535);
    data_stream << buffer;
    ...
    // Compute
    Plaintext scalar;
    encoder.encode((16.67), scale, scalar);
    Ciphertext encrypted_result;
    evaluator.multiply_plain(encrypted, scalar, encrypted_result);
    evaluator.rescale_to_next_inplace(encrypted_result);
    encrypted_result.scale() = scale;

    // Send back result
    auto size_result = encrypted_result.save(data_stream);
    cout << "Result ciphertext size: " << size_result << " bytes" << endl;
    cout << "Send result ciphertext to bergman" << endl;
    send(obj_socket, data_stream.str().c_str(), data_stream.str().length(), 0);
    cout << "Result ciphertext sent to bergman" << endl;
    // Reset data stream
    data_stream.str(string());
