...
int main() {
    ...
    reader = read(sock, buffer, 65535);
    cout << "Got parms from PID" << endl;
    parms_stream << buffer;
    cout << "Read parms into parms_stream" << endl;
    // Clear the buffer
    ...
    cout << "Create secret key" << endl;
    KeyGenerator keygen(context);
    auto secret_key = keygen.secret_key();
    cout << "Create public key" << endl;
    PublicKey public_key;
    keygen.create_public_key(public_key);

    // Generate and send relin key
    cout << "Create relin key" << endl;
    ...
    cout << "Set up encoder, encryptor, evaluator and decryptor" << endl;
    ...
    // Send ciphertext
    auto size_init_sum = init_sum.save(data_stream);
    cout << "Ciphertext size: " << size_init_sum << " bytes" << endl;
    cout << "Send ciphertext to PID" << endl;
    send(sock, data_stream.str().c_str(), data_stream.str().length(), 0);
    cout << "Ciphertext sent to PID" << endl;
    // Reset data stream
    data_stream.str(string());

    cout << "Read Ciphertext from PID" << endl;
    // Read encrypted result from server
    reader = read(sock, buffer, 65535);
    data_stream << buffer;
    // Clear the buffer
    memset(buffer, 0, sizeof(buffer));
